...
public class LtxChunkHeuristicTokenScanner extends LtxHeuristicTokenScanner {
	...
	public LtxChunkHeuristicTokenScanner(final IDocContentSections documentContentInfo) {
	...
	protected int createForwardBound(final int start) throws BadLocationException {
		...
		if (matcher.matches(ITexDocumentConstants.LTX_DEFAULT_CONTENT_TYPE)) {
			final IRegion rCodeRegion= getTexCodeRegion(start);
			if (rCodeRegion != null) {
	...
	private IRegion getTexCodeRegion(final int offset) throws BadLocationException {
